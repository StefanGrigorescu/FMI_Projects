...
//std::ifstream f("pamant.in");
//std::ofstream g("pamant.out");
...
public:
	...
	size_t get_nrVeciniNod(int nod) { return vecini[nod].size(); }
	...
	void cadruDFS(int*& ordineVizita, int& nrCompConexe, std::vector<int>& accesComponenteConexe);
	vector_vectori cadru_biconexe();
	vector_vectori cadru_tareconexe(bool &statusIsError = false);
	vector_vectori criticalConnections(int n, vector_vectori& connections, int nod = 1);
...
tmp_moneda
void graf<moneda>::cadruDFS(int *&ordineVizita, int &nrCompConexe, std::vector<int> &accesComponenteConexe)
{
	nrCompConexe = 0;
	bool* viz = new bool[nrvf + 1]{ 0 };
	if(!ordineVizita)
		ordineVizita = new int[nrvf + 1]{ 0 };
	...
	for (int i = 1; i <= nrvf; i++)
		...
		{
			nrCompConexe++;
			accesComponenteConexe.push_back(i);
			idxOrdineVizita++;
...
tmp_moneda
std::vector< std::vector< int > > graf<moneda>::criticalConnections(int n, vector_vectori& connections, int nod)
{
	vector_vectori solutia;		                // solutia, de forma unui vector cu alti vectori ce reprezinta muchiile critice
	bool* viz = new bool[nrvf + 1]{ 0 };	// nodurile vizitate deja
	stackpair muchiiviz;							// stiva de muchii vizitate
	multime* setgol = new multime;		// un set "wayback" pe care il pasez fiului pentru a-mi returna caile de intoarcere disponibile
	mCrits(nod, -1, viz, setgol, muchiiviz, connections, solutia);
...
void kruskal_paduri()
{
	...
	for (int i = 0; i < nrcmds; i++)
	{
		...
		if (cmd == 1)    // unim arborii nodurilor x si y (daca fac parte din arbori diferiti)
		{
		...
		else                 // verificam daca doua noduri fac parte din acelasi arbore
		{
			const int tataMare1 = tataMare(x, tata);
			const int tataMare2 = tataMare(y, tata);
...
// face o selectie a muchiilor arborelui partial de cost minim
tmp_moneda
...
// calculeaza costurile minime pentru drumurile de la un nod sursa la toate nodurile
tmp_moneda
...
// calculeaza matricea de costuri minime pentru drumurile de la fiecare nod la toate nodurile
tmp_moneda
...
// generam arborele BFS al unei retele de transport, selectand doar muchiile nesaturate accesibile din nodul de start; returnam vizitat[nod_destinatie]
tmp_moneda
...
// calculeaza fluxul maxim ce poate trece printr-o retea de transport
tmp_moneda
...
// returneaza un vector de perechi de int ce reprezinta muchiile cu flux nenul din reteaua de transport
tmp_moneda
...
// returneaza vector cu indecsii muchiilor (in numar minim) ce ar face graful sa fie conex 
tmp_moneda
...
// pentru problema paftenie barbarul; transforma vecinii dragonilor in dragoni
...
// pentru problema paftenie barbarul
tmp_moneda
...
int main()
{
	...
	///		/// DFS & comp conexe
	/*graf<> graful(false, false, false);
	int* ordineVizita = NULL;
	int nrCompConexe;
	std::vector<int> accesComponenteConexe;
	graful.cadruDFS(ordineVizita, nrCompConexe, accesComponenteConexe);
	g << nrCompConexe;
	if (ordineVizita)
	{
		for (int i = 1; i <= graful.get_nrvf(); i++)
	...
	*/
	...
	std::set<int> noduriCritice;

	vector_vectori* connections = new vector_vectori;
	graful.copiaza_listeAdiacenta(*connections);
	for (size_t i = 0; i < accesComponenteConexe.size(); i++)
	{
		g << accesComponenteConexe[i] << " ";
		vector_vectori muchiiCritice = graful.criticalConnections(graful.get_nrvf(), *connections, accesComponenteConexe[i]);
		for (size_t j = 0; j < muchiiCritice.size(); j++)
		{
			int nod = muchiiCritice[j][0];
			if (noduriCritice.find(nod) == noduriCritice.end() and graful.get_nrVeciniNod(nod) > 1)
				noduriCritice.insert(nod);

			nod = muchiiCritice[j][1];
			if (noduriCritice.find(nod) == noduriCritice.end() and graful.get_nrVeciniNod(nod) > 1)
				noduriCritice.insert(nod);
		}
	}
	...
	g << "\n" << noduriCritice.size() << "\n";
	for (auto it = noduriCritice.begin(); it != noduriCritice.end(); it++)
		g << *it << " ";*/
