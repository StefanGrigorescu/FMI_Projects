...
namespace in
{
	...
	public:
		...
		*/
		static InputSingleton* getInput(string pathIn)
		{
			if (_input != NULL)
			{
				return new InputSingleton(_input);
			}
			...
			_input = new InputSingleton(generationSize, xMin, xMax, closedLeft, closedRight, a, b, c, precision, crossProb, mutationProb, stepsCount);
	...
	private:
		...
		InputSingleton(InputSingleton* source):
			generationSize(source->generationSize), xMin(source->xMin), xMax(source->xMax), closedLeft(source->closedLeft), closedRight(source->closedRight), a(source->a), b(source->b), c(source->c), precision(source->precision), crossProb(source->crossProb), mutationProb(source->mutationProb), stepsCount(source->stepsCount) {}

		static InputSingleton* _input;				// Reference to the singleton instance of the InputUtility class.
