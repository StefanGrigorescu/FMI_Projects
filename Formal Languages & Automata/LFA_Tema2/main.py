    ...
    for dreapta_productiei in productii[neterminal]:
        if dreapta_productiei == cod_epsilon:   # daca productia merge in epsilon
            ...
            if poz < lungime_cuvinte:
                for pozitie in range(poz, poz + len(dreapta_productiei)-1):
                    # print("len(dreapta_productiei) pt "+neterminal+"= "+str(len(dreapta_productiei)))
                    if pozitie >= lungime_cuvinte:
                        break
                    cuvant[pozitie] = dreapta_productiei[pozitie-poz]
                poz = poz + len(dreapta_productiei)-2
                if poz >= lungime_cuvinte:
                    poz = lungime_cuvinte
                if dreapta_productiei[-1] in productii:  # daca productia merge in terminal si neterminal
                    if poz < lungime_cuvinte - 1:
